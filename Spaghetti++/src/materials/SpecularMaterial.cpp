...
SpecularMaterial::SpecularMaterial(Texture * pDiffuseTexture, Texture* pSpecularTexture):_diffuseTexture(pDiffuseTexture), _specularTexture(pSpecularTexture) {
...
void SpecularMaterial::_lazyInitializeShader() {
    if (!_shader) {
        ...
        _shader->addShader(GL_VERTEX_SHADER, config::MGE_SHADER_PATH+"specular.vs");
        _shader->addShader(GL_FRAGMENT_SHADER, config::MGE_SHADER_PATH+"specular.fs");
        _shader->finalize();
...
void SpecularMaterial::render(World* pWorld, Mesh* pMesh, const glm::mat4& pModelMatrix, const glm::mat4& pViewMatrix, const glm::mat4& pProjectionMatrix) {
